...
func TestAll(t *testing.T) {
	...
	t.Log("r1:", r1, ", r2:", r2)
	if int64(r1) == int64Max {
		t.Log("测试结果正确")
	} else {
		t.Fatal("测试结果错误，正确值为", int64Max, "，当前值:", int64(r1))
	}

	// 2个参数，类型为int64，返回为uint64
	ResultValAndParam := testdll.NewProc("ResultValAndParam")
	r1, r2, _ = ResultValAndParam.Call(uintptr(int64Val1), uintptr(int64Val2))
	t.Log("r1:", r1, ", r2:", r2)
	if uint64(r1) == uint64Max {
		t.Log("测试结果正确")
	} else {
		t.Fatal("测试结果错误，正确值为", uint64Max, "，当前值:", uint64(r1))
	}
